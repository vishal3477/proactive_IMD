...
def test(data):
    ...
    with torch.no_grad():
        ...
        signal_est_red=0.3*signal_est.clone() 
        signal_set_red=0.3*signal_set.clone() 
        
        
        signal_est_red_resize=F.interpolate(signal_est_red, size=256)
        print(signal_est_red_resize.shape)
        gen_img_with_signal,input_with_signal,real_image  = model(data_i,signal_est_red_resize, mode='inference')
    ...
    return  signal_set,real_image.type(torch.cuda.FloatTensor) ,input_with_signal, signal_rec,gen_img_with_signal.type(torch.cuda.FloatTensor) ,signal_fake, dist_fake
...
for epoch in range(epochs):
    ...
    for batch_idx_test, data_i in enumerate(dataloader):
    
        signal_set,input_images ,input_with_signal, signal_rec,gen_img_with_signal,signal_fake, dist_fake=test(data_i)
